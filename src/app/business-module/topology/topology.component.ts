...
import {getCableLevel, getTopologyName, getWiringType} from '../facility/share/const/facility.config';
...
export class TopologyComponent implements OnInit, AfterViewInit {
  ...
  section_pageBean: PageBean = new PageBean(5, 1, 1); // 分页
  section_queryCondition: QueryCondition = new QueryCondition(); // 分页条件
  cable_level = {
  ...
  getTopology(id) {
    ...
    this.$facilityService.opticCableSectionByIdForTopology({
      ...
      if (result.code !== 150109) {
        ...
        Object.keys(opticCableLevel).forEach(item => {
          if (item === result.data.opticCableLevel) {
            this.topologyList.opticCableLevel = <string>getCableLevel(this.$nzI18n, result.data.opticCableLevel);
          }
        });
        Object.keys(wiringType).forEach(item => {
          if (item === result.data.topology) {
            this.topologyList.topology = <string>getTopologyName(this.$nzI18n, result.data.topology);
          }
        });
        Object.keys(topology).forEach(item => {
          if (item === result.data.wiringType) {
            this.topologyList.wiringType = <string>getWiringType(this.$nzI18n, result.data.wiringType);
          }
        });
      }
    });
  ...
  justRefreshData() {
    ...
    this.$facilityService.getCableList(this.section_queryCondition).subscribe((result: Result) => {
  ...
  queryDeviceId(data) {
    ...
    this.className = CommonUtil.getFacilityIconClassName(data.deviceType);
