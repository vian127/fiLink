...
export class SmartLabelComponent implements OnInit {
  ...
  // 智能标签表格配置
  public label_tableConfig: TableConfig;
  // 智能标签列表分页
  public label_pageBean: PageBean = new PageBean(5);
  // 智能标签列表分页条件
  public label_queryCondition: QueryCondition = new QueryCondition();
  ...
  public label_refreshData(): void {
    ...
    this.$facilityService.getSmartLabelList(this.label_queryCondition).subscribe((result: Result) => {
      ...
      operation: [
        { // 删除
          ...
          handle: (data) => {
            ...
            this.$facilityService.deleteSmartLabelInfo(request).subscribe((result: Result) => {
              if (result.code === 0) {
