...
import {getAlarmLevel, getDeviceStatus, getDeviceType, AlarmLevelCode} from '../../facility/share/const/facility.config';
...
import {MapService} from '../../../core-module/api-service/index/map/map.service';
...
export class ShowScreenComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  /** 页面枚举使用声明 */
  alarmLevelCode: any = AlarmLevelCode;
  ...
  wsMsgAccept() {
    ...
    this.$wsService.subscibeMessage.subscribe(msg => {
      ...
      const temp = {
        ...
        LevelName: getAlarmLevel(this.$I18n, data.msg.alarmLevel),
  ...
  private getDeviceTypeCount() {
    ...
    this.$MapScreenService.getDeviceTypeCount().subscribe((res: Result) => {
      if (res.code === 0) {
        // 饼图传入数据，index为颜色角标
        const deviceData: Array<{ value: number, name: string, index: number }> = [];
        let deviceTotal = 0;
        res.data.forEach(item => {
          ...
          deviceData.push({
            ...
            name: getDeviceType(this.$I18n, item.deviceType),
    ...
    this.$MapScreenService.getDeviceStatusCount().subscribe((res: Result) => {
      if (res.code === 0) {
        ...
        res.data.forEach(item => {
          statusData.push({
            value: item.deviceNum,
            name: getDeviceStatus(this.$I18n, item.deviceStatus),
  ...
  getScreenDeviceGroup() {
    ...
    this.$MapScreenService.getScreenDeviceGroup(dateCond).subscribe((res: Result) => {
      ...
      if (res.code === 0) {
        ...
        // 通过设施ids查询设施名称
        this.$MapScreenService.getDeviceByIds(deviceIds).subscribe((resData: Result) => {
          if (resData.code === 0) {
