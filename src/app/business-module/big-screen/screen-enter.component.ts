...
export class ScreenEnterComponent implements OnInit {
   ...
   */
  showScreen() {
    // 从缓存获取系统参数，是否进入大屏全屏界面, '1'启用，'0'不启用
    const screenDisplay = JSON.parse(localStorage.getItem('displaySettings')).screenDisplay;
    if (screenDisplay === '1') {
      this.isVisible = true;
      // 这里不能使用定时器，在火狐浏览器里属于非法操作不能打开全屏 在子组件中开启全屏
      // setTimeout(() => {
      //   this.showViewScreen.shoeViewScreen();
      // }, 0);
    } else {
      this.$message.info(this.language.BigScreenNotOpen);
    }
  }
