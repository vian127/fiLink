...
export class StrategyManagementComponent implements OnInit {
  ...
  private refreshData() {
    ...
    const strategyType = {filterValue: '5', filterField: 'strategyType', operator: 'like'};
  ...
  deleteLightStrategy(data) {
    ...
    if (data && data.strategyId) {
      ...
      params = data.map(item => item.strategyId);
    }
    this.$applicationService.deleteLinkageStrategy(params).subscribe((result: ResultModel<string>) => {
      if (result.code === ResultCodeEnum.success) {
        this.refreshData();
      }
    }, () => {
  ...
  public openAddPolicyControl() {
    this.$router.navigate([routerJump.STRATEGY_ADD], {}).then();
  }
  ...
  public handPolicyDetails(data) {
    this.$router.navigate([`${routerJump.STRATEGY_DETAILS}/${data.strategyId}`], {}).then();
  }
