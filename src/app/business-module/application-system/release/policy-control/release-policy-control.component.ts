...
export class ReleasePolicyControlComponent implements OnInit {
  _dataSet = [];
  ...
  private refreshData() {
    ...
    const strategyType = {filterValue: '3', filterField: 'strategyType', operator: 'like'};
  ...
  deleteLightStrategy(data) {
    ...
    this.$applicationService.deleteInfoStrategy(params).subscribe((result: ResultModel<string>) => {
  ...
  public openAddPolicyControl() {
    this.$router.navigate([routerJump.RELEASE_WORKBENCH_ADD], {}).then();
  }
  ...
  public handPolicyDetails(data) {
    this.$router.navigate([`${routerJump.RELEASE_WORKBENCH_DETAILS}/${data.strategyId}`], {}).then();
  }
