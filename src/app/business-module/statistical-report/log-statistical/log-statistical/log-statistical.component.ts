...
import {getLogStatisticalType, getLogType, getOptType, getDangerLevel, getAllType, allTypeCode} from '../log-statistical.config';
...
export class LogStatisticalComponent implements OnInit {
  ...
  _dataSetMain = [];
  // 统计表格分页配置
  logPageBean: PageBean = new PageBean(10, 1, 1);
  // 统计表格配置
  logTableConfig: TableConfig;
  // 表格统计查询条件
  logQueryCondition: QueryCondition = new QueryCondition();
  hide = true;
  // 枚举工具
  workOrderConfig;
  // 饼图实例
  ...
  ringChartInstance;
  // 日志统计类型(默认是日志类型)
  logStatisticalData: string = 'logType';
  // 日志统计LIst
  logStatisticalList = [];
  // 时间范围
  dateRange = [];
  ...
  // modal框
  modal: any;
  logTypeDisable: boolean = true;
  optTypeDisable: boolean = false;
  dangerLevelDisable: boolean = false;
  // 操作用户
  optUser = null;
  // 操作终端
  optTerminal = null;
      ...
      operation: [
        ...
        },
        {
          ...
          handle: (currentIndex) => {
            ...
            this.$logStatistical_Service.deleteLogTemplate(param).subscribe((res: Result) => {
              if (res.code === 0) {
  ...
  initLogTableConfig() {
    ...
    this.logTableConfig = {
      ...
      handleExport: (event) => {
        ...
        if (this.logStatisticalData === 'logType') {
          this.$logStatistical_Service.logTypeExport(body).subscribe((res: Result) => {
            if (res.code === 0) {
              this.$message.success(res.msg);
            } else {
              this.$message.error(res.msg);
          ...
          this.$logStatistical_Service.operateTypeExport(body).subscribe((res: Result) => {
            if (res.code === 0) {
              this.$message.success(res.msg);
            } else {
              this.$message.error(res.msg);
          ...
          this.$logStatistical_Service.securityLevelExport(body).subscribe((res: Result) => {
            if (res.code === 0) {
              this.$message.success(res.msg);
            } else {
              this.$message.error(res.msg);
  ...
  refreshData() {
    ...
    this.$logStatistical_Service.queryLogTemplateList(this.queryCondition).subscribe((res: Result) => {
      if (res.code === 0) {
  ...
  searchLogInfo() {
    ...
    if (this.logStatisticalData === 'logType') {   // 日志类型
      ...
      this.$logStatistical_Service.queryLogTypeCount(this.logQueryCondition).subscribe((result: Result) => {
        this.logTableConfig.isLoading = false;
        this._dataList = [result.data];
        this._dataSetMain = [result.data];
        this.hide = false;
        this.setChartData(result.data, 'logType');
      ...
      this.logTableConfig.isLoading = true;
      this.$logStatistical_Service.queryOperateTypeCount(this.logQueryCondition).subscribe((result: Result) => {
        this.logTableConfig.isLoading = false;
        this._dataList = [result.data];
        this._dataSetMain = [result.data];
        this.hide = false;
        this.setChartData(result.data, 'operationalType');
        this.exportData = CommonUtil.deepClone(this._dataList);
        this.ProgressShow = false;
      }, () => {
        this.logTableConfig.isLoading = false;
      ...
      this.logTableConfig.isLoading = true;
      this.$logStatistical_Service.querySecurityTypeCount(this.logQueryCondition).subscribe((result: Result) => {
        this.logTableConfig.isLoading = false;
        this._dataList = [result.data];
        this._dataSetMain = [result.data];
        this.hide = false;
        this.setChartData(result.data, 'dangerLevelType');
        this.exportData = CommonUtil.deepClone(this._dataList);
        this.ProgressShow = false;
      }, () => {
        this.logTableConfig.isLoading = false;
  ...
  firstStatistical() {
    ...
    // this.optUser = SessionUtil.getUserInfo().userName;
    this.onChange(this.dateRange);
  }
}
