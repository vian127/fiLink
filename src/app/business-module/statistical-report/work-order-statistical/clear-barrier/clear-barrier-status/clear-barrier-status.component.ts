...
@Component({
  ...
  templateUrl: './clear-barrier-status.component.html',
  styleUrls: ['./clear-barrier-status.component.scss']
})
export class ClearBarrierStatusComponent implements OnInit {
  ...
  initTableConfig() {
    ...
    this.tableConfig = {
      ...
      handleExport: (event) => {
        ...
        this.$workOrder_Service.clearBarrierStatusExport(body).subscribe((res: Result) => {
          if (res.code === 0) {
