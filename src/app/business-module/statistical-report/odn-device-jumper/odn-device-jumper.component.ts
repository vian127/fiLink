...
export class OdnDeviceJumperComponent implements OnInit {
  ...
  deviceTwoDataSet = [];
  // 统计表格分页配置
  pageBean: PageBean = new PageBean(10, 1, 10);
  // 选择设施1 表格分页配置
  devicePageBeanOne: PageBean = new PageBean(10, 1, 10);
  // 选择设施2 表格分页配置
  deviceTwoPageBean: PageBean = new PageBean(10, 1, 10);
  // 统计表格配置
  tableConfig: TableConfig;
  // 选择设施1表格配置
  deviceTableConfig: TableConfig;
  // 选择设施2表格配置
  deviceTwoTableConfig: TableConfig;
  // 选择设施1查询条件
  deviceQueryOne = new QueryCondition();
  // 选择设施2 查询条件
  deviceQueryTwo = new QueryCondition();
  // 页面类型
  pageTypes;
  // 设施1名称
  deviceNameOne;
  ...
  queryDeviceList() {
    ...
    this.$facilityService.deviceListByPage(this.deviceQueryOne).subscribe((result: Result) => {
  ...
  queryDeviceTwoList() {
    ...
    this.$facilityService.deviceListByPage(this.deviceQueryTwo).subscribe((result: Result) => {
